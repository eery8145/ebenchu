...
class CUrlManager extends CApplicationComponent
{
	...
	public function getBaseUrl()
	{
		...
		if(!empty($cookie['myCityPy']->value)){
			$hostQian = $cookie['myCityPy']->value;
		}

		$hostArr = array('shop','images','admin');
		if(!empty($hostQian) && !in_array($hostQian, $hostArr)){
			$hostQian = 'http://'.$hostQian.'.'.$host;
		}else{
			$hostQian = '';
		}

		if($this->_baseUrl!==null)
			return $hostQian.$this->_baseUrl;
		else
		{
			if($this->showScriptName)
