...
class CUploadedFile extends CComponent
{
	...
	protected static function prefetchFiles()
	{
      ...
      if(in_array($ext,$types))//如果存在能上传的类型
		  {
		    $imagessize = getimagesize($info['tmp_name']);
		    $imagessize = explode('"', $imagessize[3]);
		    $width = $imagessize[1];
		    $height = $imagessize[3];
		  } else{
		  	$width =0;
	      $height=0;
			...
			self::collectFilesRecursive($class, $info['name'], $info['tmp_name'], $info['type'], $info['size'], $info['error'], $width, $height);
		}
	}
	/**
	 ...
	 */
	protected static function collectFilesRecursive($key, $names, $tmp_names, $types, $sizes, $errors, $width, $height)
	{
		if(is_array($names))
		{
			...
			self::$_files[$key] = new CUploadedFile($names, $tmp_names, $types, $sizes, $errors, $width, $height);
	}
	 ...
	 */
	public function __construct($name,$tempName,$type,$size,$error,$width=0,$height=0)
	{
		$this->_name=$name;
