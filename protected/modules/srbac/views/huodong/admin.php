...
$('.search-form form').submit(function(){
	$('#huodong-grid').yiiGridView('update', {
		data: $(this).serialize()
	});
	return false;
});
...
array(
  ...
  //           'filter'=>CHtml::listData(Member::model()->findAll('status = 1'), 'id', 'nickname'),
  //       ),
        array(
            'name'=>'cid',
            'value'=>'$data->HdcateOne->name',
            'filter'=>CHtml::listData(Hdcate::model()->findAll('pid != 0'), 'id', 'name'),
        ),
		'title',
		'time',
		'address',
...
<script type="text/javascript">
    ...
    var deleteAll = function (){
        ...
        if(data.length > 0){
            $.post('http://www.ebc.com/srbac/huodong/delall',{'selectdel[]':data}, function (data) {
            	var json = eval('(' + data + ')'); 
        	    if(json.statu == 0){
