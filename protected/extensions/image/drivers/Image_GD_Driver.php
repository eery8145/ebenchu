...
class Image_GD_Driver extends Image_Driver {
	...
	public function resize($properties)
	{
		...
		//原始未处理过的参数
		$myWidth = $properties['width'];
		$myHeight = $properties['height'];
		...
		if(isset($properties['liubai']) && $properties['liubai'] == 'liubai'){
			if($width>$height){
				$bili = 1-($height/$width);
				$set = ($myHeight*$bili)/2;
				$x = 0;
			...
			$img = $this->imagecreatetransparent($myWidth, $myHeight);
		}else{
			$x = 0;
			$y = 0;
			$img = $this->imagecreatetransparent($properties['width'], $properties['height']);
		...
		// Execute the resize
		if ($status = imagecopyresampled($img, $this->tmp_image, $x, $y, 0, 0, $properties['width'], $properties['height'], $width, $height))
		{
			// Swap the new image for the old one
			imagedestroy($this->tmp_image);
			$this->tmp_image = $img;
	...
	protected function imagecreatetransparent($width, $height)
	{
		...
		{
			$src = $_SERVER['DOCUMENT_ROOT']."/static/touming.png";
			// Decode the blank PNG if it has not been done already
			self::$blank_png = imagecreatefrompng($src);

			imagesavealpha(self::$blank_png,true);//这里很重要;
